...
async function initTenderPage() {
  ...
  function renderView(rows) {
    ...
    /* ── Image-1 style half-circle meter (dark bg, glow, needle) ── */
    function buildDelayMeter(delayVal, rowIdx) {
      if (isNaN(delayVal)) return '<span style="color:rgba(255,255,255,.22);font-size:.72rem">—</span>';
      const isDelayed=delayVal>0, isAhead=delayVal<0;
      const col = isDelayed ? '#f43f5e' : isAhead ? '#10b981' : '#4a5568';
      const colGlow = isDelayed ? '#f43f5e' : isAhead ? '#10b981' : 'transparent';

      const fillPct = Math.min(100, (Math.abs(delayVal) / maxDelayAbs) * 100);
      // SVG: 110×68, half-circle centred cx=55 cy=56 r=44
      const cx=55, cy=56, rad=44;
      const bgArc=`M ${cx-rad} ${cy} A ${rad} ${rad} 0 0 1 ${cx+rad} ${cy}`;
      let fillArc='';
      if (fillPct>0.5) {
      ...
      const ticks=[25,50,75,100].map(tp=>{
        const ta=Math.PI-(tp/100)*Math.PI;
        const x1=cx+(rad-6)*Math.cos(ta), y1=cy-(rad-6)*Math.sin(ta);
      ...
      const bx=cx+(nLen-8)*Math.cos(needleAngle), by=cy-(nLen-8)*Math.sin(needleAngle);
      const px=Math.sin(needleAngle)*4, py=Math.cos(needleAngle)*4;
      const gmId=`gm-dl-${rowIdx}`;
      ...
      return `<div class="inline-meter-wrap${isDelayed?' m-delayed':''}">
        <svg viewBox="0 0 110 62" width="110" height="62" style="overflow:visible;display:block">
          <defs>
            <radialGradient id="${gmId}" cx="50%" cy="100%" r="80%">
              <stop offset="0%" stop-color="${col}" stop-opacity="0.18"/>
              <stop offset="100%" stop-color="${col}" stop-opacity="0"/>
            </radialGradient>
            ${fillPct>0?`<filter id="${filtId}" x="-30%" y="-30%" width="160%" height="160%">
              <feGaussianBlur stdDeviation="3" result="blur"/>
              <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
            </filter>`:''}
          </defs>
          <!-- Dark panel bg -->
          <rect x="1" y="1" width="108" height="60" rx="7" fill="rgba(6,8,18,0.82)" stroke="${col}" stroke-opacity="0.22" stroke-width="1"/>
          <!-- Subtle radial glow behind arc -->
          ${fillPct>0?`<ellipse cx="${cx}" cy="${cy}" rx="${rad+8}" ry="${(rad+8)/2}" fill="url(#${gmId})"/>`:''}
          <!-- Track -->
          <path d="${bgArc}" fill="none" stroke="rgba(255,255,255,0.07)" stroke-width="9" stroke-linecap="round"/>
          <!-- Ticks -->
          ${ticks}
          <!-- Fill arc with glow -->
          ${fillArc?`<path d="${fillArc}" fill="none" stroke="${col}" stroke-width="9" stroke-linecap="round" stroke-opacity="0.35" filter="url(#${filtId})"/>
          <path d="${fillArc}" fill="none" stroke="${col}" stroke-width="5" stroke-linecap="round"/>`:''}
          <!-- Needle stem -->
          <line x1="${cx}" y1="${cy}" x2="${bx.toFixed(2)}" y2="${by.toFixed(2)}" stroke="${col}" stroke-width="1.8" stroke-linecap="round"/>
          <!-- Needle arrowhead -->
          <polygon points="${ntx.toFixed(2)},${nty.toFixed(2)} ${(bx-px).toFixed(2)},${(by+py).toFixed(2)} ${(bx+px).toFixed(2)},${(by-py).toFixed(2)}" fill="${col}" style="filter:drop-shadow(0 0 4px ${colGlow})"/>
          <!-- Pivot -->
          <circle cx="${cx}" cy="${cy}" r="3" fill="${col}" stroke="rgba(0,0,0,.6)" stroke-width="1.5"/>
          <!-- Value text centred on gauge -->
          <text x="${cx}" y="${cy-10}" text-anchor="middle" font-size="11" font-weight="800" fill="${col}" font-family="'Inter',monospace,sans-serif" style="filter:drop-shadow(0 0 5px ${colGlow})">${valTxt}</text>
        </svg>
        <div style="font-size:.6rem;font-weight:700;color:${lblCol};letter-spacing:.35px;line-height:1.1;margin-top:1px">${lblTxt}</div>
      </div>`;
    }
